...
function requireFinalMapElement<T extends HTMLElement>(root: ParentNode, selector: string) {
  ...
  if (!element) {
    throw new Error(`Missing v3.9 control: ${selector}`);
  }
  return element;
}
...
function parseFinalMapFeedJson(text: string): ApprovedGeoMapFeedPacket | null {
  try {
...
function parseFinalMapQAJson(text: string): MapCandidateQAReport | null {
  try {
...
function normalizeFinalMapPrecision(value: string) {
...
function decideFinalMapGate(feed: ApprovedGeoMapFeedPacket | null, qa: MapCandidateQAReport | null, reviewerName: string, reviewerNote: string, reviewWarningsAccepted: boolean) {
  if (!feed || !qa) {
    return { status: "missing_inputs" as const, reason: "Load both a v3.7 approved map feed and its v3.8 QA report." };
  }

  if (qa.sourceFeed.feedDigest !== feed.feedDigest) {
    return { status: "blocked" as const, reason: "The QA report feed digest does not match the approved map feed digest." };
  }

  if (qa.summary.status === "empty") {
    return { status: "blocked" as const, reason: "The QA report is empty and cannot authorize a final map layer." };
  }

  if (qa.summary.status === "blocked" || qa.summary.blockerFindings > 0 || qa.summary.blockedCandidates > 0) {
    return { status: "blocked" as const, reason: "The QA report has unresolved blockers." };
  }

  if (qa.summary.status === "review") {
    if (!reviewWarningsAccepted) {
      return { status: "review_required" as const, reason: "The QA report has warnings; reviewer acceptance is required before export." };
    }
    if (!reviewerName.trim() || !reviewerNote.trim()) {
      return { status: "review_required" as const, reason: "Warning-state export requires reviewer name and a review note." };
    }
  }
...
function buildFinalRecord(record: ApprovedGeoMapRecord, qaRecord: MapCandidateQAReport["checkedRecords"][number], feedDigest: string, qaDigest: string): FinalMapLayerRecord {
  const recordCore = {
    ...
    mapPrecision: normalizeFinalMapPrecision(record.mapPrecision || record.geoPrecision || "unknown"),
  ...
  return {
    ...recordCore,
    finalRecordDigest: finalMapDigest(recordCore)
  };
}

function buildFinalMapLayerPacket(feed: ApprovedGeoMapFeedPacket | null, qa: MapCandidateQAReport | null, reviewerName: string, reviewerNote: string, reviewWarningsAccepted: boolean): FinalMapLayerPacket {
  const generatedAt = new Date().toISOString();
  const gate = decideFinalMapGate(feed, qa, reviewerName, reviewerNote, reviewWarningsAccepted);
  const qaByDigest = new Map<string, MapCandidateQAReport["checkedRecords"][number]>();

  if (qa) {
    for (const checked of qa.checkedRecords) {
      qaByDigest.set(checked.mapFeedRecordDigest, checked);
    }
  }
  ...
  const finalRecords = gate.status === "export_ready" && feed
    ? feed.mapRecords
        .map((record) => ({ record, checked: qaByDigest.get(record.mapFeedRecordDigest) }))
        .filter((item): item is { record: ApprovedGeoMapRecord; checked: MapCandidateQAReport["checkedRecords"][number] } => Boolean(item.checked) && item.checked.qaStatus !== "blocked")
        .map((item) => buildFinalRecord(item.record, item.checked, sourceFeedDigest, sourceQaDigest))
    : [];
  ...
  const finalLayerCore: Omit<FinalMapLayerPacket, "finalLayerDigest"> = {
    ...
    gateDecision: {
      ...gateCore,
      gateDigest: finalMapDigest(gateCore)
    },
    sourceFeed: {
  ...
  return {
    ...finalLayerCore,
    finalLayerDigest: finalMapDigest(finalLayerCore)
  };
}

function renderFinalMapResults(container: HTMLElement, packet: FinalMapLayerPacket | null) {
  if (!packet) {
...
function mountFinalMapLayerExportGate() {
  ...
  panel.innerHTML = `
    ...
    <div class="final-map-summary" data-role="summary">
      <span>Status: none</span><span>Final records: 0</span><span>Excluded: 0</span><span>Digest: none</span>
    </div>
    <div class="final-map-boundary">
      ${FINAL_MAP_BOUNDARY.map((item) => `<span>${finalMapEscape(item)}</span>`).join("")}
    </div>
  ...
  function buildFromInputs() {
    const feed = parseFinalMapFeedJson(feedInput.value);
    const qa = parseFinalMapQAJson(qaInput.value);
    currentPacket = buildFinalMapLayerPacket(feed, qa, reviewerInput.value, reviewNoteInput.value, acceptReviewInput.checked);
    ...
    renderFinalMapResults(resultsNode, currentPacket);
    updateSummary();
  }
  ...
  clearButton.addEventListener("click", () => {
    ...
    currentPacket = null;
    localStorage.removeItem(FINAL_MAP_STORAGE_KEY);
    renderFinalMapResults(resultsNode, null);
    updateSummary();
  });
