...
function buildGuidedFlow(): PublicConsoleGuidedFlow {
  ...
  const guidedSteps = steps.map((step) => ({
    ...step,
    priority: step.step === recommended.step ? (step.status === "blocked" ? "blocked" : step.status === "ready" ? "complete" : "next") : step.status === "ready" ? "complete" : "soon"
  }));
